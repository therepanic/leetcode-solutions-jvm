...
public class ColoringABorder {
    public int[][] colorBorder(int[][] grid, int row, int col, int color) {
        ...
        bypass(grid, row, col, grid[row][col], checked);
    ...
    private void bypass(int[][] grid, int m, int n, int color, boolean[][] checked) {
        if (m < 0 || n < 0 || m >= grid.length || n >= grid[0].length || grid[m][n] != color || checked[m][n]) return;
        checked[m][n] = true;
        bypass(grid, m + 1, n, color, checked);
        bypass(grid, m - 1, n, color, checked);
        bypass(grid, m, n + 1, color, checked);
        bypass(grid, m, n - 1, color, checked);
    }
}
