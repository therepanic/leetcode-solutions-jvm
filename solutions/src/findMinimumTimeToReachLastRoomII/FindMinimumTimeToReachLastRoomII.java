package findMinimumTimeToReachLastRoomII;
...
public class FindMinimumTimeToReachLastRoomII {
    public static int minTimeToReach(int[][] moveTime) {
        ...
        while (!heap.isEmpty()) {
            ...
            int pref = poll[3] == 0 ? 1 : 2;
            if (poll[0] + 1 < moveTime.length) {
                heap.add(new int[] {poll[0] + 1, poll[1], (poll[2] < moveTime[poll[0] + 1][poll[1]] ? moveTime[poll[0] + 1][poll[1]] + pref : poll[2] + pref), poll[3] == 1 ? 0 : 1});
            }
            if (poll[1] + 1 < moveTime[0].length) {
                heap.add(new int[] {poll[0], poll[1] + 1, (poll[2] < moveTime[poll[0]][poll[1] + 1] ? moveTime[poll[0]][poll[1] + 1] + pref : poll[2] + pref), poll[3] == 1 ? 0 : 1});
            }
            if (poll[0] - 1 >= 0) {
                heap.add(new int[] {poll[0] - 1, poll[1], (poll[2] < moveTime[poll[0]- 1][poll[1]] ? moveTime[poll[0] - 1][poll[1]] + pref : poll[2] + pref), poll[3] == 1 ? 0 : 1});
            }
